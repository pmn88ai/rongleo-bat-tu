...
// NO-AUTH: inject user từ x-user-id header
const injectUser = (req, res, next) => {
    req.user = { id: req.headers['x-user-id'] || 'anonymous', role: 'user' };
    next();
};
...
// Shape user info từ req.user đã inject
...
router.get('/daily', injectUser, shapeUserInfo, async (req, res) => {
    try {
...
// GET /api/que/monthly
router.get('/monthly', injectUser, shapeUserInfo, async (req, res) => {
    try {
        const { year, month, day, hour, minute, gender, topic, forceNew } = req.query;
        if (!year || !month || !day) {
            return res.status(400).json({ error: 'Missing birth data' });
...
// GET /api/que/history
router.get('/history', injectUser, shapeUserInfo, async (req, res) => {
    try {
...
router.get('/timeline', injectUser, shapeUserInfo, async (req, res) => {
    try {
        const userId = req.userInfo.userId;
        if (!userId) return res.status(401).json({ error: 'Unauthorized' });
...
// GET /api/que/yearly
router.get('/yearly', injectUser, shapeUserInfo, async (req, res) => {
    try {
        const { year, month, day, hour, minute, gender, topic, forceNew } = req.query;
        if (!year || !month || !day) {
            return res.status(400).json({ error: 'Missing birth data' });
...
router.post('/note', injectUser, shapeUserInfo, async (req, res) => {
    try {
